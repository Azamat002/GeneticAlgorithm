...
public class Algorithm {
    ...
    protected static int getFitness(Individual iv) {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = i + 1; j < 8; j++) {
                if (iv.list.get(i).equals(iv.list.get(j)) || checkDiagonals(iv, iv.list.get(i), i, iv.list.get(j), j)) {
                    clashes += 1;
                }
            }
        }
     ...
     */
    protected static boolean checkDiagonals(Individual iv, int current, int indexOfCurrent, int other, int indexOfOther) {
        for (int i = 0; i < 8; i++) {
            if (i != indexOfCurrent && Math.abs(indexOfCurrent - i) == Math.abs(current - iv.list.get(i))) {
                return true; // Threat found
            }
            if (i != indexOfOther && Math.abs(indexOfOther - i) == Math.abs(other - iv.list.get(i))) {
                return true; // Threat found
            }
        }
